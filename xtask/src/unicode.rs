...
mod paths {
    ...
    pub const TABLES: &str = "crates/rslint_lexer/src/tables.rs";
}
...
pub fn generate_tables() -> Result<()> {
    ...
    let tokens = quote! {
        ...
        //! Run `cargo xtask unicode` and recommit this file when Unicode support has changed.
    ...
    let pretty = crate::reformat(tokens)?;

    std::fs::write(crate::project_root().join(paths::TABLES), pretty)?;
...
impl Properties {
    ...
    fn path() -> PathBuf {
        crate::project_root().join(paths::DERIVED_CORE_PROPERTIES)
    }
