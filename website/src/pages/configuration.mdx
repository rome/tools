...
- must be reachable by Rome, e.g. symbolic links might not be resolved by Rome;
- will be processed in order: from the first one to the last one;
- can override the same properties, but ultimately only the last one will be used by Rome;
