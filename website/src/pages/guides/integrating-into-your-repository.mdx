...
First, we replace any script(s) you were using to check and to reformat your files using Prettier.
...
Similar to formatting, we replace the scripts used for linting in the `package.json` file, for example:
...
Replace the `lint` and `format` scripts with Rome's alternatives in your `package.json`:
...
## npm workspaces, Yarn berry workspaces, pnpm workspaces, lerna, ...
