import { IndentStyle, PlaygroundSettings, PlaygroundState, QuoteProperties, QuoteStyle, SourceType, TrailingComma } from "../types";
...
export default function SettingsPane({
	...
	const setQuoteProperties = createSetter(setPlaygroundState, "quoteProperties");
	const setTrailingComma = createSetter(setPlaygroundState, "trailingComma");
	const setEnabledNurseryRules = createSetter(setPlaygroundState, "enabledNurseryRules");

	return (
		<div className="settings-pane">
			<FormatterSettings lineWidth={lineWidth} setLineWidth={setLineWidth} indentStyle={indentStyle} setIndentStyle={setIndentStyle} indentWidth={indentWidth} setIndentWidth={setIndentWidth} quoteStyle={quoteStyle} setQuoteStyle={setQuoteStyle} quoteProperties={quoteProperties} setQuoteProperties={setQuoteProperties} trailingComma={trailingComma} setTrailingComma={setTrailingComma} />
			<LinterSettings enabledNurseryRules={enabledNurseryRules} setEnabledNurseryRules={setEnabledNurseryRules} />
			<SyntaxSettings sourceType={sourceType} setSourceType={setSourceType} isTypeScript={isTypeScript} setIsTypeScript={setIsTypeScript} isJsx={isJsx} setIsJsx={setIsJsx} />
		</div>
	);
}

function SyntaxSettings({sourceType, setSourceType, isTypeScript, setIsTypeScript, isJsx, setIsJsx}: {
...
function FormatterSettings({
	...
	return <>
		...
		<section>
			...
			<div className={`field-row ${indentStyle === IndentStyle.Space ? "" : "disabled"}`}>
				<label htmlFor="indentWidth">
					Indent Width
				</label>
				<input
					type="number"
...
function LinterSettings({enabledNurseryRules, setEnabledNurseryRules}: {
...
function LineWidthInput({ lineWidth, setLineWidth }:  {
	lineWidth: number;
	setLineWidth: (lineWidth: number) => void;
}) {
	const [showCustom, setShowCustom] = useState(lineWidth !== 80 && lineWidth !== 120);
