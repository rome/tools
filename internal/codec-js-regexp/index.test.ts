...
import {createRegExpParser} from "@internal/codec-js-regexp/index";

test(
	"verify the parser recognizes nodes correctly",
	async (t) => {
		const node1 = createRegExpParser({
			path: "unknown",
		...
		const node2 = createRegExpParser({
			path: "unknown",
		...
		const node3 = createRegExpParser({
			path: "unknown",
			input: "look" +
