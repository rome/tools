...
type MaybeValue = RangeNameAndValue
	|	RangeValueAndName
	|	RangeValueGTValue
	|	RangeValueLTValue | undefined;


export function assertRangeNameAndValue(value: MaybeValue, parser: CSSParser): value is RangeNameAndValue {
	const token = parser.getToken();
	return token.type === 'Ident';
}

export function assertRangeValueGTValue (value: MaybeValue, parser: CSSParser): value is RangeValueGTValue {
	const token = parser.getToken();
	return token.type === "Ident" && token.value === ">";
}
export function assertRangeValueLTValue (value: MaybeValue, parser: CSSParser): value is RangeValueLTValue {
	const token = parser.getToken();
	return token.type === "Ident" && token.value === "<";
}

export function assertRangeValueAndName (value: MaybeValue, parser: CSSParser): value is RangeValueAndName {

	return false;
}


export function parseMediaFeatureRange(parser: CSSParser): CSSMediaFeatureRange | undefined {
