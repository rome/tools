...
import {lint, LintRequest, LintResult, lintRuleNames} from "@internal/compiler";
...
function createLintTransformOptions(ast: JSRoot, mutateConfig: (config: ProjectConfig) => ProjectConfig): LintRequest {
	return {
...
test("disabledLintRules single", async (t) => {
	...
	const res = await lint(createLintTransformOptions(template.root`foo;`, (config) => config));
	t.true(hasUndeclaredDiag(res));

	// Make sure when it's not disabled the diagnostic it is not present
	const res2 = await lint(createLintTransformOptions(template.root`foo;`, (config) => ({
		...config,
		lint: {
			...config.lint,
...
test("disabledLintRules all rules", async (t) => {
	const res = await lint(createLintTransformOptions(template.root`foo;`, (config) => ({
		...config,
		lint: {
			...config.lint,
...
test("format disabled", async (t) => {
	const res = await lint(createLintTransformOptions(template.root`wacky\n\tformatting( yes,\nok );`, (config) => ({
		...config,
		format: {
			...config.format,
			enabled: false,
