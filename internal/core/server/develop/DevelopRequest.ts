...
export default class DevelopRequest {
  constructor({request, response, httpServer, server}: DevelopRequestOptions) {
    ...
    if (host === undefined) {
      ...
      if (typeof addressInfo === "string") {
        ...
        throw new Error(`server.address() should not be null as in order to receive a socket we must be listening`);
      } else {
  ...
  private async maybeServeStaticFile(relative: string | RelativePath): Promise<boolean> {
    const {server} = this;
    ...
    if (knownEtag !== undefined && knownEtag.mtime === stats.mtimeMs) {
    ...
    this.server.staticETags.set(path, {
      mtime: stats.mtimeMs,
      etag: hash,
    });
  ...
  public async _handle() {
    ...
    const servedStaticIndex = await this.maybeServeStaticFile(this.pathname.append("index.html"));
