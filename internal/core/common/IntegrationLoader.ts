...
import { parseSemverRange, satisfiesSemver, SemverRangeNode, SemverVersionNode, stringifySemver } from "@internal/codec-semver";
import { Consumer, consumeUnknown } from "@internal/consume";
import {AbsoluteFilePath, AbsoluteFilePathMap, createAbsoluteFilePath} from "@internal/path";
import {json} from "@internal/codec-config";
import {readFileTextMeta} from "@internal/fs";
import {createSingleDiagnosticError, descriptions, provideDiagnosticAdviceForError} from "@internal/diagnostics";
...
type IntegrationLoaderNormalize<Value> = (consumer: Consumer, version: undefined | SemverVersionNode) => Value;
...
export default class IntegrationLoader<Value> {
  constructor({name, range, normalize}: {name: string, range?: string; normalize: IntegrationLoaderNormalize<Value>}) {
    this.loaded = new AbsoluteFilePathMap();
    this.name = name;
    this.normalize = normalize;
    this.range = range === undefined ? undefined : parseSemverRange({input: range});
  }
  ...
  private resolve(id: string, require: NodeRequire, path: AbsoluteFilePath): string {
    try {
  ...
  public async wrap<T>(callback: () => Promise<T>): Promise<T> {
    ...
    try {
      ...
      throw provideDiagnosticAdviceForError(err, {
  ...
  public async load(path: AbsoluteFilePath): Promise<IntegrationLoaderEntry<Value>> {
    ...
    if (expectedRange !== undefined) {
      const manifestPath = createAbsoluteFilePath(this.resolve(`${this.name}/package.json`, require, path));

      const jsonConsumer = json.consumeValue(await readFileTextMeta(manifestPath));
      const versionProp = jsonConsumer.get("version");

      const manifest = await normalizeManifest(manifestPath, jsonConsumer, []);

      if (manifest.version === undefined) {
        throw versionProp.unexpected(descriptions.INTEGRATIONS.MISSING_VERSION(this.name), {at: "none"});
      }

      if (!satisfiesSemver(manifest.version, expectedRange)) {
        throw versionProp.unexpected(descriptions.INTEGRATIONS.UNSUPPORTED_VERSION(this.name, stringifySemver(expectedRange)), {at: "none"});
      }
