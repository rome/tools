...
import {
    ...
    MarkdownListBlock, MarkdownListItem,
    MarkdownParagraph,
    MarkdownRoot, MarkdownText
} from "@romefrontend/ast";
import {Number0, ob1Add} from "@romefrontend/ob1";
import {isEscaped} from "@romefrontend/string-utils";
import {AnyMarkdownInlineNode, AnyMarkdownNode, MarkdownListChildren} from "@romefrontend/ast/markdown/types";
import {hasThematicBreak} from "@romefrontend/markdown-parser/utils";

export const createMarkdownParser = createParser((ParserCore, ParserOptionsWithRequiredPath) =>
   class MarkdownParser extends ParserOptionsWithRequiredPath<Tokens> {
       ...
       consumeBlock(blockChar: string, index: Number0, currentChar: string) {
           ...
           if (hasThematicBreak([currentChar, nextChar, nextNextChar].join(''))) {
               ...
               const [, endIndex] = this.readInputFrom(index, (char) => {
           ...
           if (!escaped) {
               ...
               if (isDigit(char)) {
                   ...
                   if (nextChar === "." && nextNextChar === " ") {
                       return this.finishValueToken("ListItem", "numeric-list", ob1Add(index, 3))
                   }
               }
           }
       ...
       parseListBlock(): MarkdownListBlock {
           ...
           while(!this.matchToken("EOF") && this.matchToken("ListItem")) {
               console.log('parsing block', this.getToken())

               console.log('paring item')
