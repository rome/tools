...
function hasAnchorContent(node: JSXElement): boolean {
	const dangerouslySetInnerHTML = hasJSXAttribute(node, "dangerouslySetInnerHTML")

	return (
		(
			node.children.length > 0 && 
			!node.children.some(child => child.type === "JSXElement" && hasJSXAttribute(child, "aria-hidden"))
		)||
