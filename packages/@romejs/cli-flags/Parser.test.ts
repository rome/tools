...
async function testParser<T>(t: TestHelper, {
  defineFlags,
  args,
  callback,
}: {
...
test('does not allow camel case', async (t) => {
  await testParser(t, {
    defineFlags: () => {},
    args: ['--fooBar'],
  });
...
test('flag value equals', async (t) => {
  await testParser(t, {
    defineFlags: (c) => {
      return {
...
test('required flag omitted', async (t) => {
  await testParser(t, {
    defineFlags: (c) => {
      return {
        name: c.get('name').asString(),
...
test('optional flag omitted', async (t) => {
  await testParser(t, {
    defineFlags: (c) => {
      return {
        name: c.get('name').asStringOrVoid(),
...
test('flag with description', async (t) => {
  await testParser(t, {
    defineFlags: (c) => {
      return {
        name: c.get('name', {
...
test('optional boolean flag', async (t) => {
  await testParser(t, {
    defineFlags: (c) => {
      return {
...
test('optional boolean flag omitted', async (t) => {
  await testParser(t, {
    defineFlags: (c) => {
      return {
        run: c.get('run').asBooleanOrVoid(),
...
test('required boolean flag', async (t) => {
  await testParser(t, {
    defineFlags: (c) => {
      return {
...
test('required boolean flag omitted', async (t) => {
  await testParser(t, {
    defineFlags: (c) => {
      return {
        run: c.get('run').asBoolean(),
...
test('flip boolean flag', async (t) => {
  await testParser(t, {
    defineFlags: (c) => {
      return {
        run: c.get('run').asBoolean(),
