...
type InputMessagesCategory = {[key: string]:
    | string
    | DiagnosticMetadataString
    | InputMessagesFactory};

type InputMessages = {[category: string]: InputMessagesCategory};

type OuputMessagesFactoryReturn<Ret extends DiagnosticMetadataString> =
  & Omit<Ret, 'message'>
  & {message: DiagnosticBlessedMessage};

type OutputMessagesFactory<Func extends InputMessagesFactory> = (...params: Parameters<
  Func
  ...
  return [
    {
      ...
      category: 'info',
      message: name === ''
        ? 'Originated from this opening tag' : `Originated from opening tag of <emphasis>${name}</emphasis>`,
    },
    {
      type: 'frame',
      location: openingLoc,
  ...
  BUNDLER: {
    ...
    UNKNOWN_EXPORT: (
      ...
      ({
        ...
        ] : buildSuggestionAdvice(name, exportedNames, {
          formatItem: (name) => {
    ...
    UNKNOWN_LABEL: (label: undefined | string) =>
      ({
        message: label === undefined
          ? 'No loop label found' : markup`Unknown label <emphasis>${label}</emphasis>`,
      }),
    ...
    UNEXPECTED_TOKEN: (
      ...
      ({
        message: expected === undefined
          ? 'Unexpected token' : `Unexpected token, expected ${expected}`,
