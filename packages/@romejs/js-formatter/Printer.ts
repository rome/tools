...
export default class Printer {
    ...
    if (this.lineWrap && this.canOverwriteLastUnbrokenGroup()) {
      ...
      for (const tok of token.tokens) {
        if ((tok.type === 'LinkedGroups' || tok.type === 'Group') &&
            !this.brokenGroups.has(tok) && && (tok.type !== 'Group' ||
            !tok.broken.force))) {
          firstGroup = tok;
          break;
        }
  ...
  getMappings(): Mappings {
   ...
   {
    if (this.lineWrap && this.canOverwriteLastUnbrokenGroup()) {
      let firstGroup: undefined | LinkedGroupsToken | GroupToken; // Get the first unbroken group
      for (const tok of token.tokens) {
        if ((tok.type === 'LinkedGroups' || tok.type === 'Group') &&
            !this.brokenGroups.has(tok) && (tok.type !== 'Group' ||
            !tok.broken.force)) {
  ...
  printOperatorToken(token: OperatorToken) {
    const str = token.value; // Space is mandatory to avoid outputting <!--
    // http://javascript.spec.whatwg.org/#comment-syntax
    if (str === '--' && this.state.lastBuff.endsWith('!') || // Need spaces for operators of the same kind to avoid: `a+++b`
      str[0] === '+' && this.state.lastBuff.endsWith('+') || str[0] === '-' &&
        this.state.lastBuff.endsWith('-') || // Needs spaces to avoid changing '34' to '34.', which would still be a valid number.
  ...
  printGroupToken(token: GroupToken, tokens: Tokens, index: number) {
    const {breakOnNewline, groups, priority, broken, unbroken} = token;
    const isBroken = broken.force || this.brokenGroups.has(token);
    let ourUnbrokenGroup: undefined | GroupSnapshot; // If the last broken group was a linked group then it's in charge of us, so don't catch anything
    if (!isBroken && this.canOverwriteLastUnbrokenGroup()) {
      ourUnbrokenGroup = this.createStateSnapshot(
        priority === true,
        tokens,
...
export default class Printer {
  ...
  push(str: string) {
    ...
    this.maybeAddTerminatorlessParen(str); // Only output indentation if we aren't compact
    if (!this.compact && str !== '\n' && this.state.endsWithNewline) {
      str = this.state.indentString + str;
    } // Determine if we need to line wrap. We skip this when we aren't in pretty mode for better performance.
    if (this.lineWrap) {
