...
#[must_use]
pub(crate) fn expected_token(token: JsonSyntaxKind) -> impl ToDiagnostic {
    ExpectedToken(
        token
            .to_string()
...
#[must_use]
pub(crate) fn expected_token_any(tokens: &[JsonSyntaxKind]) -> impl ToDiagnostic {
    use std::fmt::Write;
