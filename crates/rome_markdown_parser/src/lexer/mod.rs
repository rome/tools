...
use rome_markdown_syntax::{MdSyntaxKind, MdSyntaxKind::*, TextLen, TextRange, TextSize, T};
...
impl<'src> Lexer<'src> {
    ...
    fn eat_byte(&mut self, tok: MdSyntaxKind) -> MdSyntaxKind {
        self.advance(1);
        tok
    }
    ...
    /// ## Safety
    /// Must be called at a valid UT8 char boundary
    fn consume_newline_or_whitespaces(&mut self) -> MdSyntaxKind {
    ...
    fn lex_token(&mut self, current: u8) -> MdSyntaxKind {
    ...
    #[inline]
    fn eat_unexpected_character(&mut self) -> MdSyntaxKind {
        self.assert_at_char_boundary();
    ...
    fn lex_number(&mut self, current: u8) -> MdSyntaxKind {
        self.assert_at_char_boundary();
        ...
        match state {
            LexNumberState::IntegerPart
            | LexNumberState::FractionalPart
            | LexNumberState::Exponent => MD_STRING_LITERAL,
            LexNumberState::FirstDigit => {
                let err = ParseDiagnostic::new(
    ...
    fn lex_string_literal(&mut self, quote: u8) -> MdSyntaxKind {
        ...
        match state {
            LexStringState::Terminated => MD_STRING_LITERAL,
    ...
    fn lex_identifier(&mut self, first: u8) -> MdSyntaxKind {
        self.assert_at_char_boundary();
    ...
    fn lex_slash(&mut self) -> MdSyntaxKind {
        let start = self.text_position();
        match self.peek_byte() {
            Some(b'*') => {
                // eat `/*`
