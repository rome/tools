...
impl ToFormatElement for ForHead {
	fn to_format_element(&self, formatter: &Formatter) -> Option<FormatElement> {
		match self {
			...
			ForHead::GroupingExpr(_) => todo!(),
			ForHead::BracketExpr(_) => todo!(),
			ForHead::DotExpr(_) => todo!(),
			ForHead::NewExpr(_) => todo!(),
			ForHead::CallExpr(node) => node.to_format_element(formatter),
			ForHead::UnaryExpr(_) => todo!(),
			ForHead::BinExpr(_) => todo!(),
			ForHead::CondExpr(_) => todo!(),
			ForHead::AssignExpr(_) => todo!(),
			ForHead::SequenceExpr(node) => node.to_format_element(formatter),
			ForHead::FnExpr(_) => todo!(),
			ForHead::ClassExpr(_) => todo!(),
			ForHead::NewTarget(_) => todo!(),
			ForHead::ImportMeta(_) => todo!(),
			ForHead::SuperCall(node) => node.to_format_element(formatter),
			ForHead::ImportCall(_) => todo!(),
			ForHead::YieldExpr(_) => todo!(),
			ForHead::AwaitExpr(_) => todo!(),
			ForHead::PrivatePropAccess(_) => todo!(),
			ForHead::TsNonNull(_) => todo!(),
			ForHead::TsAssertion(_) => todo!(),
			ForHead::TsConstAssertion(_) => todo!(),
			ForHead::Literal(node) => node.to_format_element(formatter),
		}
	}
}
