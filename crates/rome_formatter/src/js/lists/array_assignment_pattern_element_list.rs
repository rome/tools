...
use rome_js_syntax::{JsAnyArrayAssignmentPatternElement, JsArrayAssignmentPatternElementList};
...
impl ToFormatElement for JsArrayAssignmentPatternElementList {
    fn to_format_element(&self, formatter: &Formatter) -> FormatResult<FormatElement> {
        ...
        let has_trailing_rest = match self.into_iter().last() {
            Some(elem) => matches!(
                elem?,
                JsAnyArrayAssignmentPatternElement::JsArrayAssignmentPatternRestElement(_)
            ),
            None => false,
        };
