...
const PRETTIER_ROOT = path.resolve(process.argv[2], 'tests/format/js');
        ...
        if(entry.isFile()) {
            ...
            for(const file of await fs.readdir(path.resolve(dir, '..'))) {
                ...
                if(key in snapshot && typeof snapshot[key] === 'string') {
                    ...
                    // to this file, then an absolute path using the rome_formatter
                    ...
                    await fs.writeFile(path.resolve(outDir, snapFile), snapContent);
                }
            }
        }
    }
...
async function main() {
    console.log("Extracting tests from %s ...", PRETTIER_ROOT);
    await traverseDir(PRETTIER_ROOT);
}
