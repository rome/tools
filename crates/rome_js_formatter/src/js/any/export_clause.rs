...
impl FormatRule<JsAnyExportClause> for FormatJsAnyExportClause {
    fn format(node: &JsAnyExportClause, formatter: &Formatter) -> FormatResult<FormatElement> {
        match node {
            ...
            JsAnyExportClause::JsAnyDeclarationClause(node) => formatted![formatter, node.format()],
            ...
            JsAnyExportClause::TsExportDeclareClause(node) => formatted![formatter, node.format()],
        }
    }
}
