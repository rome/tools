...
const {readGeneratedFile, write, camelCaseToKebabCase} = require("../_utils.cjs");
const {lintRulesFolder, categoriesFile, lintRulesDocFolder} = require("../_constants.cjs");
...
for (const {basename, category} of defs) {
	...
	if(fs.existsSync(ruleDocFile)){
		...
		if(!description){
			console.log(`/lint/rules/${ruleNameKebabCase}.md is missing a description\n`);
		}
...
fs.writeFile(lintRulesDocFile, docTemplate, function (err,data) {
  if (err) {
    return console.log(err);
	}

	console.log(`Wrote: ${lintRulesDocFile}`)
	console.log(`\nRules doc progress: ${countExistingDocFiles}/${countExistingDocFiles + countMissingDocFiles}`);
});